...
class MenuScene extends Scene {
  ...
  private generateNewGame: TextButton;
  ...
  create() {
    ...
    this.generateNewGame = new TextButton(
      this,
      10,
      this.renderer.height / 4 + 40,
      "Generate Dungeon",
      {
        fontSize: "25px",
        fontFamily: "main_menu-font",
      ...
      async () => {
        ...
        const result = await Swal.fire({
          ...
          preConfirm: async () => {
            let inputValidator = async (values) => {
              if (!values || !values[0]) {
                Swal.showValidationMessage("You need to select a xStart");
                return false;
              }
              if (!values || !values[1]) {
                Swal.showValidationMessage("You need to select a yStart");
                return false;
              }
              if (!values || !values[2]) {
                Swal.showValidationMessage("You need to select a xEnd");
                return false;
              }
              if (!values || !values[3]) {
                Swal.showValidationMessage("You need to select a yEnd");
                return false;
              }
              if (!values || !values[4]) {
                Swal.showValidationMessage("You need to select a config file");
            ...
            if (await inputValidator(results)) {
              dungeon = await API.generate(parseInt(results[0]), parseInt(results[1]), parseInt(results[2]), parseInt(results[3]), results[4]);
              return !!dungeon;
            }
          },
        });
    ...
    this.add.existing(this.generateNewGame);
    this.generateNewGame.setX(-this.generateNewGame.width);

    this.tweens.add({
      delay: 3500,
      duration: 1000,
      targets: [this.newGame, this.loadGame, this.credits, this.exitGame, this.generateNewGame],
      x: 10,
      ease: "Bounce",
    });
